...
const letterIterator = ['a', 'b', 'c', 'd', 'e', 'f', 'g', 'h', 'i', 'j', 'k', 'l', 'm', 'n', 'o', 'p'];
...
(function run() {
    while (true) {
        ...
        function checkLetter(enteredLetter) {
            for (let i = 0; i <= letterIterator.length; i++) {
                return enteredLetter === letterIterator[i];
            };
        }
            ...
            case 'b':
                ...
                switch (filtersMenu) {
                    ...
                    case 'a':
                        ...
                        stringJson.forEach((obj, i) => {
                            console.log((letterIterator[i] + ')' + obj.category));
                        });
                        console.log('-)To select all categories except selected');

                        let category = prompt('Select a category from the console:')
                        let selectedCategories = stringJson.filter((value) => {
                            let selectedObj = null;
                            stringJson.forEach(obj => {
                                selectedObj = obj.category;
                            });
                        })

                        if (!category.split('').find(checkLetter)) {
